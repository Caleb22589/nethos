...
def install_desktop(root: str, payload: str, username: str) -> None:
    ...
    with open(os.path.join(home, ".bash_profile"), "w") as fh:
        fh.write(
            ...
            "    # the dead buttons.\n"
            "    # Default to the GPU when there is one, matching\n"
            "    # nethos-session. This defaulted to off, and it runs\n"
            "    # before the compositor -- so the whole session\n"
            "    # inherited it and nethos-view could not override it.\n"
            "    # With compositing off WebKit repaints only the damaged\n"
            "    # rectangle and assumes an opaque backdrop, so on a\n"
            "    # transparent layer surface nothing clears what was\n"
            "    # drawn before. That is the ghosting.\n"
            "    if [ \"${NETHOS_GPU_COMPOSITING:-auto}\" = 0 ]; then\n"
            "        export WEBKIT_DISABLE_COMPOSITING_MODE=1\n"
            "    elif [ \"${NETHOS_GPU_COMPOSITING:-auto}\" = 1 ] || \\\n"
            "         ls /dev/dri/renderD* >/dev/null 2>&1; then\n"
