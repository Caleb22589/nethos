...
def install_desktop(root: str, payload: str, username: str) -> None:
    ...
    with open(os.path.join(home, ".bash_profile"), "w") as fh:
        fh.write(
            ...
            "    # Keyed off the renderer, not the presence of a device. A\n"
            "    # render node exists even when Mesa falls back to llvmpipe,\n"
            "    # and under llvmpipe accelerated compositing is slower than\n"
            "    # none: WebKit keeps extra layer buffers and blends them on\n"
            "    # the CPU. Only hardware earns it.\n"
