...
#   scripts/dev-push.sh --clear      remove it, so the stick boots stock again
#
...
Kernel changes still need a real build -- this carries the shell, the
apps, nethosd and the nethos-* tools, which is nearly everything else.
TXT
